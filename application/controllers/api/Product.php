...
class Product extends REST_Controller {
	...
	{
        $data = $this->getbase->getinfo('product', $id);
        $this->response($data, REST_Controller::HTTP_OK);
    }

    public function list_get($id = -1)
	{
        $data = $this->getbase->getarray('product', array('serviceid'=>$id));
        $this->response($data, REST_Controller::HTTP_OK);
    }
    ...
    public function index_post()
    {
        ...
        try 
        {
            $retmsg = $this->getbase->insert('product', $inpt);
            if($retmsg['code']=='0') {
    ...
    public function index_put($id)
    {
        ...
        try 
        {
            $retmsg = $this->getbase->update('product', $inpt, $id);
            if($retmsg=='0') {
    ...
    public function index_delete($id)
    {
        try 
        {
            $this->getbase->delete('product', $id);
